...
export const getBoleta = async (id_empleado, id_boleta) => {
  const response = await Axios.get(
    EMPLEADOS.BOLETAS_EMPLEADO + "/" + id_empleado + "/" + id_boleta
  );
  return response.data;
};
...
export const getPlanilla = async (mes, anio, quincena) => {
  const response = await Axios.get(
    NOMINA.GET_NOMINA + "/" + anio + "/" + mes + "/" + quincena
  );
  return response.data;
};

export const downloadPlanilla = async (mes, anio, quincena) => {
  const response = await Axios.post(
    NOMINA.DOWNLOAD_NOMINA + "/" + anio + "/" + mes + "/" + quincena,
    { responseType: "blob" }
  );
  ...
  link.download = `planilla-${+new Date()}.xlsx`;
  link.click();
};
