...
const eventLabels = Object.freeze({
  SUCCESS: {
    create: "Incapacidad creada correctamente",
    update: "Incapacidad actualizada correctamente",
    delete: "Incapacidad eliminada correctamente",
  },
  FAILED: {
...
export default function IncapaList() {
  ...
  const createEvento = useCallback(async (values, { resetForm }) => {
    setExpanded(false);
    const response = await IncapaService.create(values);
    if (response.ok) {
      resetForm();
...
function ModalEvento({
  ...
  const editEvento = useCallback(
    async (values) => {
      const response = await IncapaService.update({
        ...values,
        id_incapacidad: evento.resource.id_incapacidad,
      });
      if (response.ok) {
        handleEventModal();
        setRequestSuccess(true);
  ...
  const deleteEvento = useCallback(async () => {
    const response = await IncapaService.deleteInc({
      id_incapacidad: evento.resource.id_incapacidad,
      ...
      <Box sx={styleModal}>
        ...
        {!edit && (
          <Grid sx={{ width: "100%" }}>
            ...
            {evento.resource?.dias > 3 && (
              <>
                <Typography variant="body2" component="p">
                  Remunerado: {evento.resource?.remunerado ? "Si" : "No"}
                </Typography>
              </>
            )}
          </Grid>
